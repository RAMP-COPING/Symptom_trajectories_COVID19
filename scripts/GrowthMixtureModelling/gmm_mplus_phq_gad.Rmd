...
2. Christmas Lockdown and lockdown 3 in England (December 20, 2020 christmas lockdown, legal third lockdown began on the 6th of January. First wave of data after the christmas shut down and new national lockdown was wave 14, which began on 19 Jan. Wave 13 began on 15 December, and responses likely recorded before short christmas lockdown was announced). ***Viewing observed data and reviewing dates, better to capture the point leading to the second lockdown to capture lockdown effects. Thus ending this piece Wave 12, so that the lockdown 2&3 peice begins with 13***

  Thus piece two will be wave 6  - wave 12
  capturing the summer period where restrictions were relaxed and heading toward the worsening situation of December. Decmeber and January restrictions a potential point for change
...
3. Ease of restrictions begins (8 march 2021, slowly released over March. Follow up 16 began 16 March, thus captures period shortly after and into the easing)
  Thus piece 3 will be wave 13 to wave 14   
  Capturing the time during the third lockdown with stay at home orders and restrictions renewed
  
  
4. Piece 4 will be waves 15 & 16 capaturing the Easing of restrictions a potential inflection point. 
...
```{r calculate number of people with non missing dep}
...
dep.traj.all$phq_latentGrowthCurve_linear.out$output[277:311]  %>%   str_split("/t") %>%   kbl() %>%   kable_styling(bootstrap_options = "striped")
...
dep.traj.all$phq_latentGrowthCurve_linear.out$output[319:337]  %>%   str_split("/t") %>%   kbl() %>%   kable_styling(bootstrap_options = "striped")
```
...
dep.traj.all$phq_latentGrowthCurve_linear.out$output[598:642]  %>%   str_split("/t") %>%   kbl() %>%   kable_styling(bootstrap_options = "striped")
```
...
dep.traj.all$phq_latentGrowthCurve_linear_withpairwiseCorrelations.out$output[277:311]  %>%   str_split("/t") %>%   kbl() %>%   kable_styling(bootstrap_options = "striped")
...
dep.traj.all$phq_latentGrowthCurve_linear_withpairwiseCorrelations.out$output[365:380]  %>%   str_split("/t") %>%   kbl() %>%   kable_styling(bootstrap_options = "striped")
```
...
dep.traj.all$phq_latentGrowthCurve_linear_withpairwiseCorrelations.out$output[685:727]  %>%   str_split("/t") %>%   kbl() %>%   kable_styling(bootstrap_options = "striped")
```
...
dep.traj.all$phq_latentGrowthCurve_quadratic.out$output[286:339]  %>%   str_split("/t") %>%   kbl() %>%   kable_styling(bootstrap_options = "striped")
...
dep.traj.all$phq_latentGrowthCurve_quadratic.out$output[463:484]  %>%   str_split("/t") %>%   kbl() %>%   kable_styling(bootstrap_options = "striped")
```
...
dep.traj.all$phq_latentGrowthCurve_quadratic.out$output[654:697]  %>%   str_split("/t") %>%   kbl() %>%   kable_styling(bootstrap_options = "striped")
```
...
dep.traj.all$phq_latentGrowthCurve_quadratic_withPairwiseCorrelations.out$output[283:337]  %>%   str_split("/t") %>%   kbl() %>%   kable_styling(bootstrap_options = "striped")
...
dep.traj.all$phq_latentGrowthCurve_quadratic_withPairwiseCorrelations.out$output[545:556]  %>%   str_split("/t") %>%   kbl() %>%   kable_styling(bootstrap_options = "striped")
```
...
dep.traj.all$phq_latentGrowthCurve_quadratic_withPairwiseCorrelations.out$output[736:779]  %>%   str_split("/t") %>%   kbl() %>%   kable_styling(bootstrap_options = "striped")
```
...
dep.traj.all$phq_latentGrowthCurve_piecewise.out$output[322:409]  %>%   str_split("/t") %>%   kbl() %>%   kable_styling(bootstrap_options = "striped")
...
dep.traj.all$phq_latentGrowthCurve_piecewise.out$output[435:451]  %>%   str_split("/t") %>%   kbl() %>%   kable_styling(bootstrap_options = "striped")
```
...
dep.traj.all$phq_latentGrowthCurve_piecewise.out$output[788:830]  %>%   str_split("/t") %>%   kbl() %>%   kable_styling(bootstrap_options = "striped")
```
...
dep.traj.all$phq_latentGrowthCurve_piecewise_wPairwise_correlations.out$output[328:414]  %>%   str_split("/t") %>%   kbl() %>%   kable_styling(bootstrap_options = "striped")
...
dep.traj.all$phq_latentGrowthCurve_piecewise_wPairwise_correlations.out$output[684:700]  %>%   str_split("/t") %>%   kbl() %>%   kable_styling(bootstrap_options = "striped")
```
...
dep.traj.all$phq_latentGrowthCurve_piecewise_wPairwise_correlations.out$output[877:920]  %>%   str_split("/t") %>%   kbl() %>%   kable_styling(bootstrap_options = "striped")
```
...
dat_dep_run_2<-data.frame(matrix(nrow=15, ncol=6))
...
#### With best model from each trajectory type

Show model that performs best of each paiur of models (with or without residual correlations between contiguous data points)

Linear: with correlations
quadratic: with correlations
Piecewiser: without correlations

```{r Format Depression Growth Curve Data run 2b}

dat_dep_run_2b<-data.frame(matrix(nrow=15, ncol=3))
...
dat_dep_run_2b$Piecewise <- dep.traj.all$phq_latentGrowthCurve_piecewise.out$gh5$means_and_variances_data$y_estimated_means$values
...
dat_dep_run_2c<-data.frame(matrix(nrow=15, ncol=4))
...
dat_dep_run_2c$Piecewise <- dep.traj.all$phq_latentGrowthCurve_piecewise.out$gh5$means_and_variances_data$y_estimated_means$values
...
Note: this corresponds to the dataset with all time points, thus, different missing on all data pattern from MPlus run with time points 1,2, and 17 removed. 

#### run all models 

dont rerun models if there is already output for it (to save computational time), do show the output in the console

```{r run anxiety linear models}
...
Note that there are more people missing on all observations having dropped time points 2,3, and 17. Was 8326, but this version is 8678.
...
anx.traj.all$gad_latentGrowthCurve_linear.out$output[280:313] %>%
  str_split("/t") %>%
  kbl() %>%
...
anx.traj.all$gad_latentGrowthCurve_linear.out$output[319:337]  %>%   str_split("/t") %>%   kbl() %>%   kable_styling(bootstrap_options = "striped")
```
...
anx.traj.all$gad_latentGrowthCurve_linear.out$output[598:642]  %>%   str_split("/t") %>%   kbl() %>%   kable_styling(bootstrap_options = "striped")
```
...
anx.traj.all$gad_latentGrowthCurve_linear_withpairwiseCorrelations.out$output[277:311]  %>%   str_split("/t") %>%   kbl() %>%   kable_styling(bootstrap_options = "striped")
...
anx.traj.all$gad_latentGrowthCurve_linear_withpairwiseCorrelations.out$output[365:380]  %>%   str_split("/t") %>%   kbl() %>%   kable_styling(bootstrap_options = "striped")
```
...
anx.traj.all$gad_latentGrowthCurve_linear_withpairwiseCorrelations.out$output[685:727]  %>%   str_split("/t") %>%   kbl() %>%   kable_styling(bootstrap_options = "striped")
```
...
anx.traj.all$gad_latentGrowthCurve_quadratic.out$output[286:339]  %>%   str_split("/t") %>%   kbl() %>%   kable_styling(bootstrap_options = "striped")
...
anx.traj.all$gad_latentGrowthCurve_quadratic.out$output[463:484]  %>%   str_split("/t") %>%   kbl() %>%   kable_styling(bootstrap_options = "striped")
```
...
anx.traj.all$gad_latentGrowthCurve_quadratic.out$output[654:697]  %>%   str_split("/t") %>%   kbl() %>%   kable_styling(bootstrap_options = "striped")
```
...
anx.traj.all$gad_latentGrowthCurve_quadratic_withPairwiseCorrelations.out$output[285:339]  %>%   str_split("/t") %>%   kbl() %>%   kable_styling(bootstrap_options = "striped")
...
anx.traj.all$gad_latentGrowthCurve_quadratic_withPairwiseCorrelations.out$output[547:563]
```
...
anx.traj.all$gad_latentGrowthCurve_quadratic_withPairwiseCorrelations.out$output[738:781]  %>%   str_split("/t") %>%   kbl() %>%   kable_styling(bootstrap_options = "striped")
```
##### Piecewise (4 pieces)
...
anx.traj.all$gad_latentGrowthCurve_piecewise.out$output[322:409]  %>%   str_split("/t") %>%   kbl() %>%   kable_styling(bootstrap_options = "striped")
...
anx.traj.all$gad_latentGrowthCurve_piecewise.out$output[433:449]  %>%   str_split("/t") %>%   kbl() %>%   kable_styling(bootstrap_options = "striped")
```
...
anx.traj.all$gad_latentGrowthCurve_piecewise.out$output[788:830]  %>%   str_split("/t") %>%   kbl() %>%   kable_styling(bootstrap_options = "striped")
```
...
anx.traj.all$gad_latentGrowthCurve_piecewise_wPairwise_correlations.out$output[328:414]  %>%   str_split("/t") %>%   kbl() %>%   kable_styling(bootstrap_options = "striped")
...
anx.traj.all$gad_latentGrowthCurve_piecewise_wPairwise_correlations.out$output[682:700]  %>%   str_split("/t") %>%   kbl() %>%   kable_styling(bootstrap_options = "striped")
```
...
anx.traj.all$gad_latentGrowthCurve_piecewise_wPairwise_correlations.out$output[877:920]  %>%   str_split("/t") %>%   kbl() %>%   kable_styling(bootstrap_options = "striped")
```
...
```{r Format anxiety Growth Curve Data run 2}
...
dat_dep_run_2$Linear <-anx.traj.all$gad_latentGrowthCurve_linear.out$gh5$means_and_variances_data$y_estimated_means$values
dat_dep_run_2$LinearWithCorrelations <- anx.traj.all$gad_latentGrowthCurve_linear_withpairwiseCorrelations.out$gh5$means_and_variances_data$y_estimated_means$values

dat_dep_run_2$Quadratic <-anx.traj.all$gad_latentGrowthCurve_quadratic.out$gh5$means_and_variances_data$y_estimated_means$values
dat_dep_run_2$QuadraticWithCorrelations <- anx.traj.all$gad_latentGrowthCurve_quadratic_withPairwiseCorrelations.out$gh5$means_and_variances_data$y_estimated_means$values

dat_dep_run_2$Piecewise <- anx.traj.all$gad_latentGrowthCurve_piecewise.out$gh5$means_and_variances_data$y_estimated_means$values
dat_dep_run_2$PiecewiseWithCorrelations <- anx.traj.all$gad_latentGrowthCurve_piecewise_wPairwise_correlations.out$gh5$means_and_variances_data$y_estimated_means$values
...
growth_curves_run2 <- ggplot() +
  ...
  geom_point(data = dat_dep_run_2,
        aes(x = timepoint, y = score, colour = Model, shape=Model), show.legend = F) +
  scale_color_manual(values = pres.palette) +
  labs(x = "Follow up time point", y ="gad9 Score") +
  scale_y_continuous(expand = c(0,0), limits = c(6,10), breaks=seq(6, 10, 3)) + 
  scale_x_continuous(expand = c(0,0), limits = c(-0.5,14.5), breaks=seq(0, 14, by = 1)) +
  theme(panel.grid.major.y = element_line(size = 0.5,
                                        linetype = 'dashed',
...
#### Best model for each trajectory type

Only show the best of each pair of models (with or without pairwise residual correlations)

Linear: With correlations
Quadratic: With correlations
Piecewise: Without correlations


```{r Format anxiety Growth Curve Data run 2b}
...
dat_dep_run_2b$Linear <- anx.traj.all$gad_latentGrowthCurve_linear_withpairwiseCorrelations.out$gh5$means_and_variances_data$y_estimated_means$values
dat_dep_run_2b$Piecewise <- anx.traj.all$gad_latentGrowthCurve_piecewise.out$gh5$means_and_variances_data$y_estimated_means$values
dat_dep_run_2b$Quadratic <- anx.traj.all$gad_latentGrowthCurve_quadratic_withPairwiseCorrelations.out$gh5$means_and_variances_data$y_estimated_means$values
...
```{r Format anxiety Growth Curve Data run 2 obs est}
...
dat_dep_run_2c$Linear <- anx.traj.all$gad_latentGrowthCurve_linear_withpairwiseCorrelations.out$gh5$means_and_variances_data$y_estimated_means$values
dat_dep_run_2c$Piecewise <- anx.traj.all$gad_latentGrowthCurve_piecewise_wPairwise_correlations.out$gh5$means_and_variances_data$y_estimated_means$values
dat_dep_run_2c$Quadratic <- anx.traj.all$gad_latentGrowthCurve_quadratic_withPairwiseCorrelations.out$gh5$means_and_variances_data$y_estimated_means$values

dat_dep_run_2c$Observed <- anx.traj.all$gad_latentGrowthCurve_linear_withpairwiseCorrelations.out$gh5$means_and_variances_data$y_observed_means$values
...
# to do 
* format outputs so they are less unreadable in markdown
* format plot to show dates rather than timepoint references
