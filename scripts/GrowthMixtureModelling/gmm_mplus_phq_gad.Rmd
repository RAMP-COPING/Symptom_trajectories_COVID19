...
### create a test dataset with massively reduced rows to get Mplus running 
dataset is huge with many datapoints and MPlus doesnt give good feedback. Completely unsure if anything ia working, or if there is a problem with the script. So require a small dataset to troubleshoot mplus input scripts.

```{r make small dataset for testing}

phq_testing <- phq_check[1:40,1:6]%>%
  select(!sample) 
  
names(phq_testing) <- NULL
phq_testing[is.na(phq_testing)] <- -99

write_csv(phq_testing, "../../../data_clean/phq/phq.clean_merged_total_scores_testdata.csv")
...
* linear
* quadratic
* piecewise (with lockdown dates as variance points)
...
#
```
reducsed testfinally flagged errors!! 1) integration option doesnt work for LCGA, so removing that as it will not speed anything up.
