...
One of the models has failed to converge. (Linear trajectory with pairwise correlations). 

Tried to increase the number of iterations to 5000 (did not work) and 10000 (also did not work). Tried to specify random start value (STARTS = 40), also didnt help, although it dramatically increased run time.


#### check if any residual variances are negative and significant {.tabset}
...
#### Check loadings of indicators and intercepts are 1, and estimates relate to specified timepoints, intercept 0 for all and correlations look sensible {.tabset}

Print output file for each correlation for examination of these details and any other relavant information
...
dat_dep<-data.frame(matrix(nrow=18, ncol=3))

colnames(dat_dep)<-c("Linear", "Quadratic",  "Piecewise3")

#  "LinearCor", "QuadraticCor","Piecewise4", "Piecewise4Cor",, "Piecewise3Cor"

dat_dep$Linear <-dep.traj.all$..MPlus_input_lcga.PHQ.phq_latentGrowthCurve_linear.out$gh5$means_and_variances_data$y_estimated_means$values

#dat_dep$LinearCor<-dep.traj.all$..MPlus_input_lcga.PHQ.phq_latentGrowthCurve_linear_withpairwiseCorrelations.out$gh5$means_and_variances_data$y_estimated_means$values

dat_dep$Quadratic<-dep.traj.all$..MPlus_input_lcga.PHQ.phq_latentGrowthCurve_quadratic.out$gh5$means_and_variances_data$y_estimated_means$values

#dat_dep$QuadraticCor<-dep.traj.all$..MPlus_input_lcga.PHQ.phq_latentGrowthCurve_quadratic_withPairwiseCorrelations.out$gh5$means_and_variances_data$y_estimated_means$values

#dat_dep$Piecewise4<-dep.traj.all$..MPlus_input_lcga.PHQ.phq_latentGrowthCurve_piecwewise_version1_4pieces.out$gh5$means_and_variances_data$y_estimated_means$values

#dat_dep$Piecewise4Cor<-dep.traj.all$..MPlus_input_lcga.PHQ.phq_latentGrowthCurve_piecwewise_version1_4pieces_wPairwise_correlations.out$gh5$means_and_variances_data$y_estimated_means$values


dat_dep$Piecewise3<-dep.traj.all$..MPlus_input_lcga.PHQ.phq_latentGrowthCurve_piecwewise_version2_3pieces.out$gh5$means_and_variances_data$y_estimated_means$values

#dat_dep$Piecewise3Cor<-dep.traj.all$..MPlus_input_lcga.PHQ.phq_latentGrowthCurve_piecwewise_version2_3pieces_wPairwiseCorrelations.out$gh5$means_and_variances_data$y_estimated_means$values


dat_dep$timepoint<-c(0:17) 

dat_dep<-dat_dep%>%pivot_longer(-timepoint, names_to = "Model", values_to = "score")%>%mutate(Model = factor(Model, levels = c("Linear","Quadratic","Piecewise3")))
