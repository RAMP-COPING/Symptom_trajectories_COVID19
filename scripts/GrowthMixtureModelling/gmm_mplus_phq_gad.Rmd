...
The covariance coverage between some time points is lower than MPlus default (10%). This prevents algorithm for missing data being applied (MLR) and standard errors cannot be computed, even when setting the coverage expectations to zero. 

Looking at our data structure and MPlus output, I suspect this is due to missing data from COPING survey for waves 2 & 3 resulting in perfect zero covariance coverage between waves 2 & 3 with wave 17 (COPING only). 

To resolve this, will run the model without the two RAMP only waves
...
anx.traj.all$gad_latentGrowthCurve_linear.out$output[761:813]  %>%   str_split("/t") %>%   kbl() %>%   kable_styling(bootstrap_options = "striped")
```
...
anx.traj.all$gad_latentGrowthCurve_linear_withpairwiseCorrelations.out$output[854:798]  %>%   str_split("/t") %>%   kbl() %>%   kable_styling(bootstrap_options = "striped")
```
