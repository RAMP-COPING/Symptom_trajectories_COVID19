---
title: "Run Latent Class Growth Analyses for RAMP longitudinal analyses of common mental health symptoms: GAD and PHQ, and MASQ (due to CFA indicating tripartite model not a good fit for data"
...
1. Observed and estimated means: Showing model estimated means from linear, quadratic, and piecewise trajectories (solid lines) overlaid on the observed means from our sample (dashed line)
2. Plot estimated mean trajectories for every model run (with and without pairwise correlations)
3. plot estimated mean trajectories for the best of each pair of model (i.e. only including te version of the model where contiguous time point residuals were allowed to correlate as these were the best fit for the data)   
...
growth_curves_run2c <- ggplot() +
  ...
  scale_y_continuous(expand = c(0,0), limits = c(5,7), breaks=seq(5, 7, 3)) + 
  scale_x_continuous(expand = c(0,0), limits = c(-0.5,15.5), breaks=seq(0, 15, by = 1),
                   labels = timepoint_list) +
  theme(panel.grid.major.y = element_line(size = 0.5,
                                        linetype = 'dashed',
...
dat_dep_run_2b$Piecewise <- anx.traj.all$gad_latentGrowthCurve_piecewise_wPairwise_correlations.out$gh5$means_and_variances_data$y_estimated_means$values
...
Note: this corresponds to the dataset with all time points, thus, different missing on all data pattern from MPlus run with time points 1,2 and baseline
...
anx.traj.all <- readModels("./MPlus_input_lcga/masq", 
                           recursive=FALSE)
...
model <- anx.traj.all$masq_latentGrowthCurve_linear.out$parameters$unstandardized
model[model$paramHeader == "Residual.Variances",]
...
model <- anx.traj.all$masq_latentGrowthCurve_linear_withpairwiseCorrelations.out$parameters$unstandardized
model[model$paramHeader == "Residual.Variances",]
...
model <- anx.traj.all$masq_latentGrowthCurve_quadratic.out$parameters$unstandardized
model[model$paramHeader == "Residual.Variances",]
...
model <- anx.traj.all$masq_latentGrowthCurve_quadratic_withPairwiseCorrelations.out$parameters$unstandardized
model[model$paramHeader == "Residual.Variances",]
...
model <- anx.traj.all$masq_latentGrowthCurve_piecewise.out$parameters$unstandardized
model[model$paramHeader == "Residual.Variances",]
...
model <- anx.traj.all$masq_latentGrowthCurve_piecewise_wPairwise_correlations.out$parameters$unstandardized
model[model$paramHeader == "Residual.Variances",]
...
anx.traj.all$masq_latentGrowthCurve_linear.out$output[290:327] %>%
  str_split("/t") %>%
  kbl() %>%
  kable_styling(bootstrap_options = "striped")
...
anx.traj.all$masq_latentGrowthCurve_linear_withpairwiseCorrelations.out$output[293:330]  %>%   str_split("/t") %>%   kbl() %>%   kable_styling(bootstrap_options = "striped")
...
anx.traj.all$masq_latentGrowthCurve_quadratic.out$output[298:353]  %>%   str_split("/t") %>%   kbl() %>%   kable_styling(bootstrap_options = "striped")
...
anx.traj.all$masq_latentGrowthCurve_quadratic_withPairwiseCorrelations.out$output[297:352]  %>%   str_split("/t") %>%   kbl() %>%   kable_styling(bootstrap_options = "striped")
...
anx.traj.all$masq_latentGrowthCurve_piecewise.out$output[340:432]  %>%   str_split("/t") %>%   kbl() %>%   kable_styling(bootstrap_options = "striped")
...
anx.traj.all$masq_latentGrowthCurve_piecewise_wPairwise_correlations.out$output[558:648]  %>%   str_split("/t") %>%   kbl() %>%   kable_styling(bootstrap_options = "striped")
...
anx.traj.all$masq_latentGrowthCurve_linear.out$output[335:352]  %>%   str_split("/t") %>%   kbl() %>%   kable_styling(bootstrap_options = "striped")
```
###### Linear with pairwise correlations 
All is fine
```{r check intercepts are zero lin pairwiseanx}

anx.traj.all$masq_latentGrowthCurve_linear_withpairwiseCorrelations.out$output[383:400]  %>%   str_split("/t") %>%   kbl() %>%   kable_styling(bootstrap_options = "striped")
```
###### Quadratic
All is fine
```{r check intercepts are zero quadanx}

anx.traj.all$masq_latentGrowthCurve_quadratic.out$output[366:383]  %>%   str_split("/t") %>%   kbl() %>%   kable_styling(bootstrap_options = "striped")
```
###### Quadratic with pairwise correlations
All is fine
```{r check intercepts are zero quad pairwiseanx}

anx.traj.all$masq_latentGrowthCurve_quadratic_withPairwiseCorrelations.out$output[410:427]  %>%
  str_split("/t") %>%
  kbl() %>%
  kable_styling(bootstrap_options = "striped")
```
...
anx.traj.all$masq_latentGrowthCurve_piecewise.out$output[458:475]  %>%   str_split("/t") %>%   kbl() %>%   kable_styling(bootstrap_options = "striped")
```
###### Piecewise with pairwise correlations
All is fine
```{r check intercepts are zero 4piece coranx}

anx.traj.all$masq_latentGrowthCurve_piecewise_wPairwise_correlations.out$output[718:736]  %>%   str_split("/t") %>%   kbl() %>%   kable_styling(bootstrap_options = "striped")
```
...
anx.traj.all$masq_latentGrowthCurve_linear.out$output[653:705]  %>%   str_split("/t") %>%   kbl() %>%   kable_styling(bootstrap_options = "striped")
```
###### Linear with pairwise correlations 
All is fine
```{r check correlations lin pairwiseanx}

anx.traj.all$masq_latentGrowthCurve_linear_withpairwiseCorrelations.out$output[746:798]  %>%   str_split("/t") %>%   kbl() %>%   kable_styling(bootstrap_options = "striped")
```
###### Quadratic
All is fine
```{r check correlations quadanx}

anx.traj.all$masq_latentGrowthCurve_quadratic.out$output[709:760]  %>%   str_split("/t") %>%   kbl() %>%   kable_styling(bootstrap_options = "striped")
```
###### Quadratic with pairwise correlations
All is fine
```{r check correlations quad pairwiseanx}

anx.traj.all$masq_latentGrowthCurve_quadratic_withPairwiseCorrelations.out$output[851:798]  %>%   str_split("/t") %>%   kbl() %>%   kable_styling(bootstrap_options = "striped")
```
###### Piecewise 
All is fine
```{r check correlations 4pieceanx}

anx.traj.all$masq_latentGrowthCurve_piecewise.out$output[854:906]  %>%   str_split("/t") %>%   kbl() %>%   kable_styling(bootstrap_options = "striped")
```
###### Piecewise with pairwise correlations
All is fine
...
anx.traj.all$masq_latentGrowthCurve_piecewise_wPairwise_correlations.out$output[948:1000]  %>%   str_split("/t") %>%   kbl() %>%   kable_styling(bootstrap_options = "striped")
```
...
dat_dep_run_2c$Linear <- anx.traj.all$masq_latentGrowthCurve_linear_withpairwiseCorrelations.out$gh5$means_and_variances_data$y_estimated_means$values
dat_dep_run_2c$Piecewise <- anx.traj.all$masq_latentGrowthCurve_piecewise_wPairwise_correlations.out$gh5$means_and_variances_data$y_estimated_means$values
dat_dep_run_2c$Quadratic <- anx.traj.all$masq_latentGrowthCurve_quadratic_withPairwiseCorrelations.out$gh5$means_and_variances_data$y_estimated_means$values

dat_dep_run_2c$Observed <- anx.traj.all$masq_latentGrowthCurve_linear_withpairwiseCorrelations.out$gh5$means_and_variances_data$y_observed_means$values
...
growth_curves_run2c <- ggplot() +
  ...
  geom_point(data = dat_dep_run_2c,
        aes(x = timepoint, y = score, colour = Model, shape=Model), show.legend = F) +
  scale_color_manual(values = pres.palette) +
  labs(x = "Follow up time point", y ="masq7 Score") +
  scale_y_continuous(expand = c(0,0), limits = c(5,7), breaks=seq(5, 7, 3)) + 
  scale_x_continuous(expand = c(0,0), limits = c(-0.5,15.5), breaks=seq(0, 15, by = 1),
                   labels = timepoint_list) +
  theme(panel.grid.major.y = element_line(size = 0.5,
...
dat_dep_run_2$Linear <-anx.traj.all$masq_latentGrowthCurve_linear.out$gh5$means_and_variances_data$y_estimated_means$values
dat_dep_run_2$LinearWithCorrelations <- anx.traj.all$masq_latentGrowthCurve_linear_withpairwiseCorrelations.out$gh5$means_and_variances_data$y_estimated_means$values

dat_dep_run_2$Quadratic <-anx.traj.all$masq_latentGrowthCurve_quadratic.out$gh5$means_and_variances_data$y_estimated_means$values
dat_dep_run_2$QuadraticWithCorrelations <- anx.traj.all$masq_latentGrowthCurve_quadratic_withPairwiseCorrelations.out$gh5$means_and_variances_data$y_estimated_means$values

dat_dep_run_2$Piecewise <- anx.traj.all$masq_latentGrowthCurve_piecewise.out$gh5$means_and_variances_data$y_estimated_means$values
dat_dep_run_2$PiecewiseWithCorrelations <- anx.traj.all$masq_latentGrowthCurve_piecewise_wPairwise_correlations.out$gh5$means_and_variances_data$y_estimated_means$values
...
dat_dep_run_2b$Linear <- anx.traj.all$masq_latentGrowthCurve_linear_withpairwiseCorrelations.out$gh5$means_and_variances_data$y_estimated_means$values
dat_dep_run_2b$Piecewise <- anx.traj.all$masq_latentGrowthCurve_piecewise_wPairwise_correlations.out$gh5$means_and_variances_data$y_estimated_means$values
dat_dep_run_2b$Quadratic <- anx.traj.all$masq_latentGrowthCurve_quadratic_withPairwiseCorrelations.out$gh5$means_and_variances_data$y_estimated_means$values
