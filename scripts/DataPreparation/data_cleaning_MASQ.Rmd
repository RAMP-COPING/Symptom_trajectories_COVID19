...
COPING_followupA_ong.id.long %>% 
  freq(masq.pandemic_felt_successful_.Wave_05_unc)
```
...
masq.COPING.reordered <- masq.COPING %>% 
  select(order(names(masq.COPING)))
...
## drop duplicates introduced in the follow up merge.

```{r drop follow u duplicate rows}

masq.totals.clean <- masq.totals.clean %>%
    group_by(ID) %>% 
    filter(!duplicated(ID,
                      fromLast = TRUE)
           ) %>%
...
write.dat(masq.mplus, "../../../data_clean/masq/masq.clean_merged_total_scores.dat")
write_csv(masq.mplus.names, "../../../data_clean/masq/masq.clean_merged_total_scores.dat/masq.clean_merged_total_scores_codebook.csv")
...
save_file <- file.path(dirname(dirname(getwd())),"output/masq.all_waves_summary_table.html")
