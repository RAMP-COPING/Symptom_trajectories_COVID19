...
data_phq_renamed <- data_raw %>% #new dataset with ID
  ...
  select(
         ...
         phq9.pandemic_felling_like_a_failure = phq.feeling_bad_failure_family,
         phq9.pandemic_trouble_concentrating = phq.trouble_concentrating_newspaper_reading,
         phq9.pandemic_figeting_or_moving_slowly = phq.moving_fidgety_opposite_slowly,
...
data_phq_renamed <- data.frame(apply(data_phq_renamed,2,function(x) ifelse(x==-99,NA,x)))
